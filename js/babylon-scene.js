// js/babylon-scene.js
// ------------------------------------------------------------
// Babylon Szene wird erst gestartet, wenn startBabylonScene()
// aufgerufen wird (z.B. nach Klick auf Start-Button)
// ------------------------------------------------------------

window.startBabylonScene = function () {

  const canvas = document.getElementById("renderCanvas");
  if (!canvas) {
    console.error("Canvas #renderCanvas fehlt.");
    return;
  }

  if (!window.BABYLON) {
    console.error("Babylon.js fehlt. Prüfe CDN Einbindung.");
    return;
  }
...
// Kamera fixieren:
camera.inputs.clear();      // keine Maussteuerung
...
camera.wheelPrecision = 0; 
  ...
  const startY = 2.0;
  const targetCount = projects.length || 6;
  ...
  for (let i = 0; i < targetCount; i++) {
    ...
    const box = BABYLON.MeshBuilder.CreateBox("target_" + p.id, {
      width: 2.2,
      height: 2.2,
      depth: 0.8
    }, scene);
    ...
    const stand = BABYLON.MeshBuilder.CreateBox("stand_" + p.id, {
      width: 2.6,
      height: 0.4,
      depth: 1.2
    }, scene);
  ...
  // Ray 1000 Einheiten nach vorne
  const farPoint = ray.origin.add(ray.direction.scale(1000));
  return farPoint.subtract(camera.position).normalize();
}
  ...
  function shootBall() {
    ...
    const ball = BABYLON.MeshBuilder.CreateSphere("ball", { diameter: 0.55 }, scene);
